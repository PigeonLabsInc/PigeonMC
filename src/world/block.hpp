...
struct BlockInfo {
    ...
    u8 light_level;
    bool collidable;
    BlockInfo() : id(0), solid(false), transparent(true), hardness(0), resistance(0), light_level(0), collidable(false) {}
    BlockInfo(BlockId id, const std::string& name, bool solid = true, bool transparent = false, f32 hardness = 1.0f, f32 resistance = 1.0f, u8 light_level = 0, bool collidable = true)
        : id(id), name(name), solid(solid), transparent(transparent), hardness(hardness), resistance(resistance), light_level(light_level), collidable(collidable) {}
};
